...
右键托盘图标（可用环境变量 `PRIVACY_SCREEN_LANG=zh|en` 强制语言）：

```
☑ 开启遮罩 / Enable privacy          ← 顶部独立开关，点击切换
──────────────
   选择屏幕 / Select screen    →  ☑ 全部屏幕 / All screens
                                  ☑ 显示器 1  \\.\DISPLAY1 [主]
                                  ☐ 显示器 2  \\.\DISPLAY2
──────────────
   选择图片文件夹… / Choose image folder…
   恢复纯黑 / Back to black
──────────────
   状态 / Status
   退出 / Exit
...
- 顶部开关点击切换开/关，**记忆上次的屏幕选择**；
- "选择屏幕"子菜单动态枚举显示器（标注主屏），勾选=正在覆盖；点选某屏只遮那块，再点一次取消；全部屏幕项勾选表示全屏覆盖中；
- 图片文件夹选择弹出系统对话框，ON 状态实时切换幻灯片。
